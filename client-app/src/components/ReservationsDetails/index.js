...
const ReservationsForCustomer = () => {
    ...
    if (isCustomer) {
        customerRecords = reservationData?.slice(
            indexOfFirstRecord,
            indexOfLastRecord
        );
        nPages = Math.ceil(reservationData.length / recordsPerPage);
    } else {
        hotelOwnerRecords = reservationData?.slice(
            indexOfFirstRecord,
            indexOfLastRecord
        );
        nPages = Math.ceil(reservationData.length / recordsPerPage);
                    ...
                    {showSecretField ? <Row style={{marginBottom: "20px"}}>
                            <Col md={4} style={{paddingRight: "0"}}>
                                <Input type="search" className={styles.searchButton}
                                       onChange={(e) => setSecret(e.target.value)} value={secret}
                                />
                            </Col>
